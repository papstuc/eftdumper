...
std::uint8_t* utilities::get_module(const char* module_name)
{
	...
	return reinterpret_cast<std::uint8_t*>(module_handle);
}
...
{
	std::uint8_t* module_handle = utilities::get_module(module_name);
...
std::uint8_t* utilities::pattern_scan(std::uint8_t* module, const char* signature)
{
	if (!module)
	{
		return nullptr;
	...
	std::uint32_t size_of_image = nt_headers->OptionalHeader.SizeOfImage;
	std::vector<std::uint32_t> pattern_bytes = pattern_to_byte(signature);
	std::uint8_t* image_base = reinterpret_cast<std::uint8_t*>(module);
